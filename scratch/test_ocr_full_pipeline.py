...
import winrt.windows.globalization as glob_rt
...
async def _native_windows_ocr_async(pil_img, lang_tag="en-US"):
    ...
    try:
        lang = glob_rt.Language(lang_tag)
...
def run_native_ocr_sync(pil_img, lang_tag="en-US"):
    try:
    ...
    if loop.is_running():
        ...
        with concurrent.futures.ThreadPoolExecutor() as pool:
            return pool.submit(lambda: asyncio.run(_native_windows_ocr_async(pil_img, lang_tag))).result()
    else:
        return loop.run_until_complete(_native_windows_ocr_async(pil_img, lang_tag))

print("Native OCR helper defined successfully.")
